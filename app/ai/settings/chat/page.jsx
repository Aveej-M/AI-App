...
const ChatInterface = () => {
  ...
  return (
    <>
      <button
        className="h-[40] w-[80] bg-green-500 hover:bg-green-600 text-white rounded transition ease-in shadow absolute right-4 top-[-52]"
      >Save</button>
      
      <div className="flex p-5 w-full gap-5">
      <div className="flex w-[60%] overflow-x-auto scrollbar-hide mb-[200]">
              ...
              {({ open }) => (
                <div className="relative min-w-[160px]">
                  <Listbox.Button className={`flex items-center w-full h-[40px] px-4 border ${open ? 'border-2 border-green-400' :'border-gray-300'} rounded text-[14px] text-black text-left relative`}>
                    ...
                    <Image 
                      className={`absolute right-2.5 pointer-events-none transition-transform duration-300 ${
                        open ? 'rotate-180' : ''
                      }`}
                      src="/Header/drop.png"
                      alt="Drop icon"
            ...
            <div className="flex items-center gap-4 flex-wrap mt-1">
              ...
              {(showAll ? avatars : avatars.slice(0, 4)).map((avatar, index) => (
                  <img
                    ...
                    className={`w-[50px] h-[50px] rounded-full border-2 p-1 cursor-pointer ${
                      selectedAvatar === avatar ? "border-green-500" : "border-gray-300"
                    }`}
                    alt={`avatar-${index}`}
                  />
              ))}
       ...
       {/*---------- MESSAGE CONTAINER --------- */}
      <div className="container-border h-[70%] w-[35%] relative">              
